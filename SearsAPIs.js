/*
  other things to add: caching option
*/
...
function SearsAPI ( initObj ) {
  var apiKey = initObj.apiKey,
      ...
      rApiURL = 'http://api.developer.sears.com/v1/',
  ...
  // Product Search API
  this.productSearch = function ( opts ) {
    var apiURL,
        searchOpts = {
          store: 'Sears',
          catalogId: '12605',
    ...
    apiURL = webApiURL + 'productsearch?' + prepareString( $.extend(searchOpts, opts) );
    
    if ( ! apiRequestCache[ apiURL ] ) {
       apiRequestCache[ apiURL ] = $.Deferred(function( defer ) {
         $.ajax({
           url : apiURL,
           dataType : 'json'
    ...
    return apiRequestCache[ apiURL ];
  };
  
  // Store Locator API
  this.storeLocator = function ( opts ) {
    var apiURL,
        searchOpts = {
          store : 'Sears',
          authID: searsAuthID,
    ...
    apiURL = webApiURL + 'StoreLocator?' + prepareString( $.extend(searchOpts, opts) );
  ...
  // Product Details API
  this.productDetails = function ( opts ) {
    var apiURL,
        searchOpts = {
          store : 'Sears',
          catalogId : 12605,
    ...
    apiURL = webApiURL + 'productdetails?' + prepareString( $.extend(searchOpts, opts) );
